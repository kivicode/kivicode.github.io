...
(function() {
  ...
  MT("class",
     ...
     "  [property varructor]([def x], [def y]) {",
  ...
  MT("anonymous_class_expression",
     "[keyword var] [def Adder] [operator =] [keyword class] [keyword extends] [variable Arithmetic] {",
     "  [property add]([def a], [def b]) {}",
     "};");

  MT("named_class_expression",
     "[keyword var] [def Subber] [operator =] [keyword class] [def Subtract] {",
  ...
  MT("import_dynamic",
     "[keyword var] [def t] [operator =] [keyword import]([string 'baz']).[property then]")

  MT("var",
     "[keyword function] [def f]() {",
     "  [keyword var] [[ [def a], [def b] ]] [operator =] [[ [number 1], [number 2] ]];",
     "}");
  ...
  MT("let_scoping",
     ...
     "  [keyword if] ([atom true]) { [keyword var] [def k]; [variable-2 k]; } [variable k];",
     "}");
  ...
  MT("leaving_scope",
     ...
     "  {",
     "    [keyword var] [def x] [operator =] [number 1]",
  ...
  MT("async_assignment",
     "[keyword var] [def foo] [operator =] [keyword async] [keyword function] ([def args]) { [keyword return] [atom true]; };");
  ...
  MT("async_arrow",
     "[keyword var] [def foo] [operator =] [keyword async] ([def args]) [operator =>] { [keyword return] [atom true]; };");
  ...
  MT("async_variable",
     "[keyword var] [def async] [operator =] {[property a]: [number 1]};",
     "[keyword var] [def foo] [operator =] [string-2 `bar ${][variable async].[property a][string-2 }`];")
  ...
  TS("typescript_interface_with_var",
     "[keyword var] [def hello]: {",
  ...
  TS("typescript_type_when_not_keyword",
     ...
     "  [property varructor]([def type]: [type string]) {",
  ...
  TS("typescript_complex_type_casting",
     "[keyword var] [def giftpay] [operator =] [variable config].[property get]([string 'giftpay']) [keyword as] { [[ [variable platformUuid]: [type string] ]]: { [property version]: [type number]; [property apiCode]: [type string]; } };")
  ...
  TS("modifiers",
     ...
     "  [property varructor]([keyword readonly] [keyword private] [def x]) {}",
     "}")
  ...
  TS("enum body",
     "[keyword export] [keyword var] [keyword enum] [def CodeInspectionResultType] {",
